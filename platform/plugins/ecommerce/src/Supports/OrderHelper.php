...
class OrderHelper
{
    ...
    public function downloadInvoice($order)
    {
        ...
        if (view()->exists('plugins/ecommerce/invoice::template')) {
            $invoice = PDF::loadView('plugins/ecommerce/invoice::template', compact('order'), [], 'UTF-8');
        } else {
            $invoice = PDF::loadView('plugins/ecommerce::invoices.template', compact('order'), [], 'UTF-8');
        }
    ...
    public function processHistoryVariables($history)
    {
        ...
        $variables = [
            'order_id'  => Html::link(route('orders.edit', $history->order->id), get_order_code($history->order->id) . ' <i class="fa fa-external-link-alt"></i>', ['target' => '_blank'], null, false)
                ->toHtml(),
